...
import tf2_ros
# Configuration 
from modelos_cinematicos.cfg import IK_3GConfig as Ik_3g
# CLass
...
class Inverse_Kinematics_3dof(object):
	...
	def move_dynamic_position(self):
		...
		def callback_ready(Ik_3g, level):
			openbot_object=Inverse_Kinematics_3dof()
			dynamixel=mover_Dynamixel()
			rospy.loginfo("""Reconfigure Request: {Position_X}, {Position_Z},{Angle_qy}, {LOWER_elbow}""".format(**Ik_3g))
			L1=0.094
			L2=0.112
			L3=0.15

			if "{Ready}".format(**Ik_3g)=="True":  
					parameters=[float("{Position_X}".format(**Ik_3g)),float("{Position_Z}".format(**Ik_3g)),float("{Angle_qy}".format(**Ik_3g)),L1,L2,L3,"{LOWER_elbow}".format(**Ik_3g)]
					...
					if result == "Error":
						...
						if "{Dynamixel}".format(**Ik_3g)=="True":
							print("Moving Dynamixel....") 
							dynamixel.main()
							if float("{Position_X}".format(**Ik_3g)) > 0 and float("{Position_X}".format(**Ik_3g)) >= 0.15:
								q1=-result[0]+0.17
								q2=-result[1]+0.05
							elif float("{Position_X}".format(**Ik_3g)) < 0 and float("{Position_X}".format(**Ik_3g)) <= -0.15:
								q1=-result[0]-0.17
								q2=-result[1]-0.05
							else:
								q1=-result[0]
							...
							#dynamixel.movimiento_inicial('no', result[0] ,result[1] ,'no',result[2],'no')
			else:
				pass
			return Ik_3g

		srv = Server(Ik_3g, callback_ready)
		rospy.spin()
