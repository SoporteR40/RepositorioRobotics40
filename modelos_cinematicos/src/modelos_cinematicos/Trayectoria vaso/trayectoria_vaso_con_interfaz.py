...
# Configuration 
from modelos_cinematicos.cfg import vasoConfig as vaso
...
(5)Salir""")
			...
			if desicion==1:
				...
				self.mover_dynamixel_inicial()
				print("Moviendo robot a posicion inicial...")
				input()
				self.menu()
			elif desicion == 5:
				print("Saliendo....")
	...
	def trayectoria_simulada(self,N,Tf,x1,y1,z1,x2,y2,z2,qyg):    
		#Dimensiones de Openbotv1
		L1=0.33
		L2 = 0.094
		L3 = 0.115
	...
	def girar_motor_izquierda(self):
		...
		th60 = splinecub_2(dxl6_present_position0[0],0,10,n0)

		for k in range (0,n0):
			# Add Dynamixel#1 goal position value to the Syncwrite parameter storage
			param_goal_position = [DXL_LOBYTE(DXL_LOWORD(th60[k])), DXL_HIBYTE(DXL_LOWORD(th60[k])), DXL_LOBYTE(DXL_HIWORD(th60[k])), DXL_HIBYTE(DXL_HIWORD(th60[k]))]
	...
	def girar_motor_derecha(self):
		n0=300  #Podrian dejarse fijas (?)
		dxl6_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL6_ID, ADDR_MX_PRESENT_POSITION)
		th60 = splinecub_2(dxl6_present_position0[0],1023,10,n0)
	...
	def cerrar_pinza_2(self):
		n0=300
		dxl5_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL4_ID, ADDR_MX_PRESENT_POSITION)
		th50 = splinecub_2(dxl5_present_position0[0],610,10,n0)
		for k in range (0,n0):
			# Add Dynamixel#1 goal position value to the Syncwrite parameter storage
			param_goal_position = [DXL_LOBYTE(DXL_LOWORD(th50[k])), DXL_HIBYTE(DXL_LOWORD(th50[k])), DXL_LOBYTE(DXL_HIWORD(th50[k])), DXL_HIBYTE(DXL_HIWORD(th50[k]))]
			dxl_addparam_result = groupSyncWrite.addParam(DXL5_ID,param_goal_position)
	...
	#Es para llevar el motor a su primer valor de x,y,z
	...
	#Mover cada uno de los puntos de x,y,z
	def mover_dynamixel(self,puntos):
		for posicion in puntos:
			if posicion[0] != 'none' or posicion[0] != 512 :
				# Add Dynamixel#1 goal position value to the Syncwrite parameter storage
				...
				if dxl_addparam_result != True:
					print("[ID:%03d] groupSyncWrite addparam failed" % DXL1_ID)
					quit()
			if posicion[1] != 'none' or posicion[1] != 512:
				# Add Dynamixel#2 goal position value to the Syncwrite parameter storage
				...
				if dxl_addparam_result != True:
					print("[ID:%03d] groupSyncWrite addparam failed" % DXL2_ID)
					quit()
			if posicion[2] != 'none' or posicion[2] != 512:
				# Add Dynamixel#3 goal position value to the Syncwrite parameter storage
			...
			if posicion[3] != 'none' or posicion[3] != 512:
				# Add Dynamixel#3 goal position value to the Syncwrite parameter storage
	...
	#Llevar robot a la posicion inicial
	def mover_dynamixel_inicial(self):
		n0=100
		dxl1_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL1_ID, ADDR_MX_PRESENT_POSITION)
		th10 = splinecub_2(dxl1_present_position0[0],512,10,n0)
		dxl2_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL2_ID, ADDR_MX_PRESENT_POSITION)
		th20 = splinecub_2(dxl2_present_position0[0],512,10,n0)
		dxl3_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL3_ID, ADDR_MX_PRESENT_POSITION)
		th30 = splinecub_2(dxl3_present_position0[0],512,10,n0)
		dxl4_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL4_ID, ADDR_MX_PRESENT_POSITION)
		th40 = splinecub_2(dxl4_present_position0[0],512,10,n0)
		dxl5_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL5_ID, ADDR_MX_PRESENT_POSITION)
		th50 = splinecub_2(dxl5_present_position0[0],512,10,n0)
		dxl6_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL6_ID, ADDR_MX_PRESENT_POSITION)
		th60 = splinecub_2(dxl6_present_position0[0],512,10,n0)
	...
	def dejar_vaso(self):
		...
		if desicion == 1:
			...
			self.trayectoria_real(100,10,x1=-0.28,y1=0.06,z1=0.14,x2=-0.28,y2=0.06,z2=0.01,qyg=10)
			self.abrir_pinza()
	...
	def Recoger_vaso(self):
		...
		if desicion == 1:
			...
			self.abrir_pinza()
			#self.girar_motor_izquierda()
			#input()
			#self.girar_motor_derecha()
			input()
			#Arriba
			self.trayectoria_real(100,10,x1=-0.28,y1=0.06,z1=0.14,x2=-0.28,y2=0.06,z2=0.01,qyg=10)
			input()
			self.trayectoria_real(100,10,x1=-0.26,y1=0.06,z1=0.01,x2=-0.3,y2=0.03,z2=0.02,qyg=10)
			input()
			self.cerrar_pinza()
			input()
			self.trayectoria_real(100,10,x1=-0.2,y1=-0.1,z1=0.15,x2=-0.12,y2=-0.02,z2=0.12,qyg=10)
			desicion=int(input('¿Desea soltar el vaso?: )'))
			if desicion == 1 :
				self.abrir_pinza()

			desicion=int(input('¿Desea poner el vaso?: )'))
			if desicion == 1 :
				self.cerrar_pinza()
	...
	def Recoger_vaso_2(self):
		...
		if desicion == 1:
			...
			self.cerrar_pinza()
			input()
			self.trayectoria_real(100,10,x1=-0.3,y1=0.03,z1=0.02,x2=-0.19,y2=0.2,z2=0.10,qyg=10)
			input()
			self.girar_motor_izquierda()
			input()
			desicion=int(input('¿Desea soltar el vaso?: )'))
	...
	def Recoger_vaso_3(self):
		...
		if desicion == 1:
			...
			input()
			#Arriba
			self.trayectoria_real(100,10,x1=-0.28,y1=0.06,z1=0.14,x2=-0.29,y2=0.03,z2=0.03,qyg=10)
			input()
			self.cerrar_pinza_2()
			input()
			self.trayectoria_real(100,10,x1=-0.2,y1=-0.1,z1=0.15,x2=-0.12,y2=-0.02,z2=0.12,qyg=10)
	...
	def Recoger_vaso_simulado(self):
		...
		self.move_init_pos()
		self.trayectoria_simulada(5,10,x1=-0.28,y1=0.06,z1=0.14,x2=-0.28,y2=0.06,z2=0.01,qyg=10)
		input()
		self.trayectoria_simulada(5,10,x1=-0.28,y1=0.06,z1=0.01,x2=-0.3,y2=0.03,z2=0.02,qyg=10)
		input()
		self.trayectoria_simulada(5,10,x1=-0.3,y1=0.03,z1=0.02,x2=-0.2,y2=-0.1,z2=0.15,qyg=10)
		print('hasta aqui')
		input()
		self.trayectoria_simulada(5,10,x1=-0.2,y1=-0.1,z1=0.15,x2=-0.12,y2=-0.02,z2=0.12,qyg=10)
		input("Simulaciòn terminada, presione enter para regresar a menu")
		self.Recoger_vaso()
