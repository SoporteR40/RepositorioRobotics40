...
from modelos_cinematicos.cfg import workspace_3g_qyConfig as workspace_3g
# CLass
...
class workspace_3gdl_qy(object):
	...
	def Rviz_workspace_completo(self,coordenadas):
		...
		markerArray = MarkerArray()
		marker=Marker()
		marker.header.frame_id = "arm2_link_1"#Es el parametro del robot respecto al cual hace el trazo
		marker.type = marker.SPHERE
		marker.action = marker.ADD
		marker.scale.x = -0.02
		marker.scale.y = 0.01
	...
	def Openbot_demo_movimiento(self,qy):
		if qy<=90:
			qy=90-qy
		elif qy>90:
			qy=(qy-90)*-1
		...
		for i in range(V):
			self.mover_demo(q1[i], q2[i],q3[i])
	...
	def move_dynamic_position(self):
		...
		def callback_ready(workspace_3g, level):
			openbot_object=workspace_3gdl_qy()
			dynamixel=mover_Dynamixel()
			L1=0.094
			L2=0.112
			L3=0.15

			if "{Workspace}".format(**workspace_3g)=="True":
				qy=float("{Angle_qy}".format(**workspace_3g))
				self.move_init_pos()
			...
			if "{Workspace}".format(**workspace_3g)=="False" and "{Demo}".format(**workspace_3g)=="True" and  "{Ready}".format(**workspace_3g)=="False":
				qy=float("{Angle_qy}".format(**workspace_3g))
				self.Openbot_demo_movimiento(qy)

			if "{Grafica_Python}".format(**workspace_3g)=="True":
				qy=float("{Angle_qy}".format(**workspace_3g))
				Graficas=Graficas_Espacio_de_trabajo()
				Graficas.grafica_workspace_3g_qy(qy)


			if "{Ready}".format(**workspace_3g)=="True":  
					parameters=[float("{Position_X}".format(**workspace_3g)),float("{Position_Z}".format(**workspace_3g)),float("{Angle_qy}".format(**workspace_3g)),L1,L2,L3]
					...
					if result == "Error":
						...
						id=0
						markerArray,marker=workspace_3gdl_qy.Create_markers()
						markerArray.markers.append(marker)
