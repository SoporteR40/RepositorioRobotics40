...
class Dynamixel_trayectoria(object):
    ...
    def punto_inicial_trayectoria(self,q1,q2,q3,conversiones):
        n0=100

        if q1!='no':
            p1=int( 512-conversiones[0][0]*1023/(5/3*pi) ) #Q1
            dxl2_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL2_ID, ADDR_MX_PRESENT_POSITION)
            th20 = splinecub_2(dxl2_present_position0[0],p1,10,n0)
        if q2!='no':
            p2=int( 512-conversiones[0][1]*1023/(5/3*pi) ) #Q2
            dxl3_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL3_ID, ADDR_MX_PRESENT_POSITION)
            th30 = splinecub_2(dxl3_present_position0[0],p2,10,n0)
        if q3!='no':
            p3=int( 512+conversiones[0][2]*1023/(5/3*pi) )#Q3
            dxl4_present_position0 = packetHandler.read2ByteTxRx(portHandler, DXL5_ID, ADDR_MX_PRESENT_POSITION)
            th50 = splinecub_2(dxl4_present_position0[0],p3,10,n0)
    ...
    def mover_dynamixel(self,posicion):
        print(posicion)
        ...
        if posicion[1] != 'no' :
            ...
            dxl_addparam_result = groupSyncWrite.addParam(DXL3_ID,param_goal_position_2)
            if dxl_addparam_result != True:
                print("[ID:%03d] groupSyncWrite addparam failed" % DXL3_ID)
                quit()
        if posicion[2] != 'no' :
            ...
            dxl_addparam_result = groupSyncWrite.addParam(DXL5_ID,param_goal_position_3)
            if dxl_addparam_result != True:
                print("[ID:%03d] groupSyncWrite addparam failed" % DXL5_ID)
                quit()	

        param_goal_position_4 = [DXL_LOBYTE(DXL_LOWORD(512)), DXL_HIBYTE(DXL_LOWORD(512)), DXL_LOBYTE(DXL_HIWORD(512)), DXL_HIBYTE(DXL_HIWORD(512))]			
        dxl_addparam_result = groupSyncWrite.addParam(DXL1_ID,param_goal_position_4)
        if dxl_addparam_result != True:
            print("[ID:%03d] groupSyncWrite addparam failed" % DXL1_ID)
            quit() 
        
        param_goal_position_5 = [DXL_LOBYTE(DXL_LOWORD(512)), DXL_HIBYTE(DXL_LOWORD(512)), DXL_LOBYTE(DXL_HIWORD(512)), DXL_HIBYTE(DXL_HIWORD(512))]			
        dxl_addparam_result = groupSyncWrite.addParam(DXL4_ID,param_goal_position_5)
        if dxl_addparam_result != True:
